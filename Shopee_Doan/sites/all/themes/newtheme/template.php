...
function newtheme_process_html(&$variables) {
  ...
  if (module_exists('color')) {
    ...
    $element_google_manager = array(
        '#type' => 'markup',
        '#markup' => isset($conf['google_conversion_all_page'])?$conf['google_conversion_all_page']:'',
    );

    $variables['scripts'] .= drupal_render($element_google_manager);
    //finish page
    if(arg(0) == 'payment' && arg(1) == 'finish' && arg(2) != '') {


        $element_succ = array(
            '#type' => 'markup',
            '#markup' => isset($conf['google_conversion_finish'])?$conf['google_conversion_finish']:'',
        );
